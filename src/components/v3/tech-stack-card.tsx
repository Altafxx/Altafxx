  ...
  return (
    <Card className="h-full v3-card-depth bg-gradient-to-br from-stone-800/95 to-stone-900/95 border-stone-600/50 backdrop-blur-sm overflow-hidden">
      <div className="h-full flex flex-col p-6">
        {/* Header */}
        <div className="mb-6">
          <h2 className="text-xl font-bold text-stone-50 mb-2">Tech Stack</h2>
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="grid grid-cols-4 gap-4">
            {stacks.map((stack, index) => (
              <div
                key={index}
                className="group relative p-3 rounded-lg bg-stone-700/40 hover:bg-stone-700/50 cursor-pointer transition-all duration-300 hover:scale-105"
              ...
              >
                ...
                {/* Tooltip */}
                {hoveredIndex === index && (
                  <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-stone-900 text-stone-100 text-xs px-2 py-1 rounded whitespace-nowrap z-10">
                    {stackNames[index]}
                  </div>
                )}
              </div>
